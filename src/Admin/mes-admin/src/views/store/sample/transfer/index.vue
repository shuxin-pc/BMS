<template>
  <div class="sample-gift-transfer-management">
    <!-- 搜索区域 -->
    ...
    <div class="card">
      ...
      >
        ...
        <el-table-column label="总数量" width="100" align="center">
          <template #default="{ row }">
            {{ formatNumber(row.items.reduce((sum: number, item: SampleGiftTransferItem) => sum + item.quantity, 0)) }}
          </template>
        </el-table-column>
        <el-table-column label="状态" width="100">
          <template #default="{ row }">
            <el-tag :type="sampleGiftTransferStatusTagType[row.status as SampleGiftTransferStatus]" size="small" effect="dark">
              {{ sampleGiftTransferStatusMap[row.status as SampleGiftTransferStatus] }}
            </el-tag>
          </template>
        </el-table-column>
        ...
        <el-row :gutter="16">
          <el-col :span="12">
            <el-form-item label="调出门店" prop="fromStoreId">
              <el-select v-model="formData.fromStoreId" placeholder="请选择调出门店" filterable style="width: 100%" @change="handleFromStoreChange">
                <el-option v-for="store in storeOptions" :key="store.id" :label="store.name" :value="store.id" />
              </el-select>
            </el-form-item>
          </el-col>
    ...
    >
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="状态">
          <el-tag :type="sampleGiftTransferStatusTagType[detailData.status]" size="small" effect="dark">
            {{ sampleGiftTransferStatusMap[detailData.status] }}
          </el-tag>
...
import {
  getSampleGiftTransferList,
  getSampleGiftTransferDetail,
  createSampleGiftTransfer,
  executeSampleGiftTransfer,
  cancelSampleGiftTransfer,
  getStoreOptions,
  getFromStoreProducts,
  getFromStoreProductBatches,
  sampleGiftTransferStatusMap,
  sampleGiftTransferStatusTagType,
  productTypeMap
} from '@/api/sample-gift-transfer'
import { useSystemConfigStore } from '@/stores/systemConfig'
import type { SampleGiftTransfer, SampleGiftTransferItem, SampleGiftTransferStatus, SampleGiftProductType, SampleGiftTransferProductOption, SampleGiftTransferBatchOption } from '@/api/sample-gift-transfer/types'
...
const searchForm = reactive({
  transferNo: '',
  status: undefined as SampleGiftTransferStatus | undefined,
  productType: undefined as SampleGiftProductType | undefined,
...
const tableData = ref<SampleGiftTransfer[]>([])
...
const productOptionsByFromStore = ref<Record<string, SampleGiftTransferProductOption[]>>({})
// 商品批次选项缓存（key = `${fromStoreId}_${productId}`），手动模式下使用
const batchOptionsByProduct = ref<Record<string, SampleGiftTransferBatchOption[]>>({})
...
const loadData = async () => {
  ...
  try {
    const res = await getSampleGiftTransferList({
...
// 调出门店变更：清空已选明细 + 清空批次缓存 + 加载新调出门店商品列表
const handleFromStoreChange = () => {
  formData.items = []
...
const handleSubmit = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        await createSampleGiftTransfer({
...
const detailData = reactive({
  ...
  toStoreName: '',
  transferDate: '',
  status: 1 as SampleGiftTransferStatus,
  operatorName: '',
  remark: '',
  items: [] as SampleGiftTransferItem[]
})

// 查看详情
const handleViewDetail = async (row: SampleGiftTransfer) => {
  try {
    const detail = await getSampleGiftTransferDetail(row.id)
...
const handleExecute = async (row: SampleGiftTransfer) => {
  try {
    ...
    await executeSampleGiftTransfer(row.id)
...
const handleCancel = async (row: SampleGiftTransfer) => {
  try {
    await ElMessageBox.confirm(
      `确定要取消调拨单 "${row.transferNo}" 吗？已调入的调拨单不可取消，需走反向调拨单。`,
      '确认操作',
      { type: 'warning', confirmButtonText: '确定', cancelButtonText: '返回' }
    )
    await cancelSampleGiftTransfer(row.id)
...
<style scoped>
.sample-gift-transfer-management {
  width: 100%;
}
