...
public static class InventoryLogSourceTypes
{
    ...
    /// <summary>【出库】项目卡核销出库（核销时扣减零售商品/BOM 耗材，必绑 OrderId，禁止手动创建）</summary>
    public const int TreatmentCardOutbound = 7;

    /// <summary>【出库】样品领用出库（SampleGiftReceive 触发，非销售，不计入销售额与主营成本）</summary>
    public const int SampleReceiveOutbound = 8;

    /// <summary>【出库】赠品活动出库（订单内赠品扣减，非销售，不计入销售额与主营成本）</summary>
    public const int GiftOutbound = 9;

    /// <summary>【出库】其他（手工录入或未分类的出库来源，仅用于 Type=2 出库；入库来源已不再使用此值）</summary>
    public const int Other = 10;

    /// <summary>
    /// 判定来源类型值是否为当前有效的白名单值（仅包含当前在用的常量）
    /// </summary>
    /// <param name="sourceType">来源类型值</param>
    /// <returns>合法返回 true，否则 false</returns>
    public static bool IsValid(int sourceType) =>
        sourceType == SalesOutbound
        || sourceType == PurchaseInbound
        || sourceType == ReturnInbound
        || sourceType == CheckAdjustment
        || sourceType == TransferInbound
        || sourceType == TransferOutbound
        || sourceType == PurchaseReturnOutbound
        || sourceType == TreatmentCardOutbound
    ...
    /// 包括 SampleReceiveOutbound、GiftOutbound
    /// </summary>
