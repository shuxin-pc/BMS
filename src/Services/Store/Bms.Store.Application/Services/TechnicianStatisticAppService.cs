...
public class TechnicianStatisticAppService : ITechnicianStatisticAppService
{
    ...
    /// 过滤条件：TenantId + TechnicianId 非空 + TechnicianSource=1（商家技师）+ Order.Status!=4（排除已取消）
    ...
    public async Task<ApiResponseDto<TechnicianStatReportDto>> GetReportAsync(TechnicianStatisticQueryDto query)
    {
        ...
        var returnCustomerQuery = from x in baseQuery
                                  where x.o.CustomerId.HasValue
                                  group x by new { TechnicianId = x.oi.TechnicianId!.Value, CustomerId = x.o.CustomerId!.Value } into g
                                  where g.Count() > 1
                                  group g by g.Key.TechnicianId into rg
                                  select new { TechnicianId = rg.Key, ReturnCustomerCount = rg.Count() };

        // 4. 执行查询：分页取主指标，再按当前页技师ID取回头客数据
        ...
        var returnStats = await returnCustomerQuery
            .Where(rc => mainStats.Select(ms => ms.TechnicianId).Contains(rc.TechnicianId))
            .ToListAsync();
