...
public static class SystemSeedData
{
    ...
    public static void Initialize(SystemDbContext context, ISnowflakeIdGenerator idGenerator, IPasswordHasher passwordHasher)
    {
        ...
        // 将菜单权限分配给超级管理员角色（只分配叶子节点，与页面操作逻辑一致）
        // 页面分配菜单权限时只勾选具体功能菜单，不勾选父级目录
        var leafMenuKeys = new[] { "system:user", "system:role", "system:menu", "system:organization",
            "system:tenant", "system:auditLog", "system:config", "system:subsystem" };
        foreach (var key in leafMenuKeys)
        {
            if (menuIdMap.TryGetValue(key, out var menuId))
            {
    ...
    private static Dictionary<string, long> CreateDefaultMenus(SystemDbContext context, ISnowflakeIdGenerator idGenerator, long subsystemId)
    {
        ...
        // 用户管理按钮
        CreateButtonMenu(context, idGenerator, now, userMenuId, "新增用户", "system:user:add", 1, subsystemId);
        CreateButtonMenu(context, idGenerator, now, userMenuId, "编辑用户", "system:user:edit", 2, subsystemId);
        CreateButtonMenu(context, idGenerator, now, userMenuId, "删除用户", "system:user:delete", 3, subsystemId);
        CreateButtonMenu(context, idGenerator, now, userMenuId, "批量删除", "system:user:batchDelete", 4, subsystemId);
        CreateButtonMenu(context, idGenerator, now, userMenuId, "重置密码", "system:user:resetPwd", 5, subsystemId);

        // 角色管理按钮
        CreateButtonMenu(context, idGenerator, now, roleMenuId, "新增角色", "system:role:add", 1, subsystemId);
        CreateButtonMenu(context, idGenerator, now, roleMenuId, "编辑角色", "system:role:edit", 2, subsystemId);
        CreateButtonMenu(context, idGenerator, now, roleMenuId, "删除角色", "system:role:delete", 3, subsystemId);
        CreateButtonMenu(context, idGenerator, now, roleMenuId, "批量删除", "system:role:batchDelete", 4, subsystemId);
        CreateButtonMenu(context, idGenerator, now, roleMenuId, "查看详情", "system:role:view", 5, subsystemId);
        CreateButtonMenu(context, idGenerator, now, roleMenuId, "保存权限", "system:role:saveAuth", 6, subsystemId);

        // 菜单管理按钮
        CreateButtonMenu(context, idGenerator, now, menuMenuId, "新增菜单", "system:menu:add", 1, subsystemId);
        CreateButtonMenu(context, idGenerator, now, menuMenuId, "编辑菜单", "system:menu:edit", 2, subsystemId);
        CreateButtonMenu(context, idGenerator, now, menuMenuId, "删除菜单", "system:menu:delete", 3, subsystemId);

        // 组织架构按钮
        CreateButtonMenu(context, idGenerator, now, orgMenuId, "新增组织", "system:organization:add", 1, subsystemId);
        CreateButtonMenu(context, idGenerator, now, orgMenuId, "新增子级", "system:organization:addChild", 2, subsystemId);
        CreateButtonMenu(context, idGenerator, now, orgMenuId, "编辑组织", "system:organization:edit", 3, subsystemId);
        CreateButtonMenu(context, idGenerator, now, orgMenuId, "删除组织", "system:organization:delete", 4, subsystemId);

        // 租户管理按钮
        CreateButtonMenu(context, idGenerator, now, tenantMenuId, "新增租户", "system:tenant:add", 1, subsystemId);
        CreateButtonMenu(context, idGenerator, now, tenantMenuId, "编辑租户", "system:tenant:edit", 2, subsystemId);
        CreateButtonMenu(context, idGenerator, now, tenantMenuId, "删除租户", "system:tenant:delete", 3, subsystemId);
        CreateButtonMenu(context, idGenerator, now, tenantMenuId, "批量删除", "system:tenant:batchDelete", 4, subsystemId);
        CreateButtonMenu(context, idGenerator, now, tenantMenuId, "查看详情", "system:tenant:view", 5, subsystemId);
        CreateButtonMenu(context, idGenerator, now, tenantMenuId, "保存子系统", "system:tenant:saveSubsystems", 6, subsystemId);

        // 审计日志按钮
        CreateButtonMenu(context, idGenerator, now, auditLogMenuId, "查看详情", "system:auditLog:view", 1, subsystemId);
        CreateButtonMenu(context, idGenerator, now, auditLogMenuId, "导出日志", "system:auditLog:export", 2, subsystemId);
        CreateButtonMenu(context, idGenerator, now, auditLogMenuId, "清空历史", "system:auditLog:clearHistory", 3, subsystemId);

        // 系统配置按钮
        CreateButtonMenu(context, idGenerator, now, configMenuId, "新增配置", "system:config:add", 1, subsystemId);
        CreateButtonMenu(context, idGenerator, now, configMenuId, "编辑配置", "system:config:edit", 2, subsystemId);
        CreateButtonMenu(context, idGenerator, now, configMenuId, "删除配置", "system:config:delete", 3, subsystemId);

        // 子系统管理按钮
        CreateButtonMenu(context, idGenerator, now, subsystemMenuId, "新增子系统", "system:subsystem:add", 1, subsystemId);
        CreateButtonMenu(context, idGenerator, now, subsystemMenuId, "编辑子系统", "system:subsystem:edit", 2, subsystemId);
        CreateButtonMenu(context, idGenerator, now, subsystemMenuId, "删除子系统", "system:subsystem:delete", 3, subsystemId);
        CreateButtonMenu(context, idGenerator, now, subsystemMenuId, "分配菜单", "system:subsystem:assignMenu", 4, subsystemId);
